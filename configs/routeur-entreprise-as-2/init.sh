...
# création du tunnel avec le protocole GRE entre 2 routeur
ip tunnel add tun0 mode gre local 120.0.32.3 remote 120.0.32.4
# ajout d'une ip sur "l'interface" du tunnel
ip link set tun0 up
ip addr add 10.0.0.1/30 dev tun0
ip route add 192.168.30.0/24 dev tun0
# On autorise les entrées-sorties dans le tunnel
iptables -A FORWARD -i tun0 -j ACCEPT
iptables -A FORWARD -o tun0 -j ACCEPT
# autoriser les paquets encapsulées avec le protocole GRE à entrer
